...
import (
	...
	jobApi "github.com/equinor/radix-job-scheduler/api"
	apiErrors "github.com/equinor/radix-job-scheduler/api/errors"
	jobMock "github.com/equinor/radix-job-scheduler/api/mock"
...
func setupTest(handler jobApi.Handler) *test.ControllerTestUtils {
...
func TestGetJobs(t *testing.T) {
	t.Run("Get jobs - success", func(t *testing.T) {
		...
		defer ctrl.Finish()
		jobHandler := jobMock.NewMockHandler(ctrl)
		now := time.Now()
		activeFrom := metav1.NewTime(now.Add(1 * time.Minute))
		completionTime := metav1.NewTime(now.Add(10 * time.Minute))
		radixBatchStatus := models.RadixBatchStatus{
			Name:         "jobName",
			CreationTime: now.String(),
			Status: v1.RadixBatchStatus{
				...
				Condition: v1.RadixBatchCondition{
					...
					Reason:         string(v1.BatchJobPhaseSucceeded),
					Message:        "test message",
					ActiveTime:     &activeFrom,
		...
		jobHandler.
			EXPECT().
			GetRadixBatchSingleJobStatuses().
			Return([]models.RadixBatchStatus{radixBatchStatus}, nil).
			Times(1)
		...
		if response != nil {
			...
			var returnedJobs []models.RadixBatchStatus
			test.GetResponseBody(response, &returnedJobs)
			assert.Len(t, returnedJobs, 1)
			assert.EqualValues(t, radixBatchStatus, returnedJobs[0])
			assert.Equal(t, len(radixBatchStatus.Status.JobStatuses), len(returnedJobs[0].Status.JobStatuses))
			assert.EqualValues(t, radixBatchStatus.Status, returnedJobs[0].Status)
		}
	})
	...
	t.Run("Get jobs - status code 500", func(t *testing.T) {
		...
		radixBatchStatus := models.RadixBatchStatus{
			...
			Status: v1.RadixBatchStatus{
				...
				Condition: v1.RadixBatchCondition{
					Type:           v1.BatchConditionTypeCompleted,
					Reason:         string(v1.BatchJobPhaseFailed),
...
func TestGetJob(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		jobHandler.
			EXPECT().
			GetRadixBatchSingleJobStatus(jobName).
		...
		if response != nil {
			assert.Equal(t, http.StatusOK, response.StatusCode)
			var returnedJob models.RadixBatchStatus
			test.GetResponseBody(response, &returnedJob)
			assert.Equal(t, radixBatchStatus.Name, returnedJob.Name)
			assert.EqualValues(t, radixBatchStatus, returnedJob)
			assert.Equal(t, len(radixBatchStatus.Status.JobStatuses), len(returnedJob.Status.JobStatuses))
			assert.EqualValues(t, radixBatchStatus.Status, returnedJob.Status)
		}
	})
